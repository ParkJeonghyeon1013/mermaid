...
def minigame1_1 ():
    print("현재 채취 현황 : 미역 {0}개 / 따개비 + 거북손 {1}개".format(mini1_NPC.seaweed_count, mini1_NPC.shell_count))
    print("[{0}]인어: 으악!! 미역이 흘러나온 미역 양식장 주인이 작살로 공격하려고 해요!! 작살에 쓰인 문자를 입력해 3초 내로 반격해주세요.".format(mermaid.name))
    input("\n------------------------------------------------------------------------------\n")
...
def minigame1_3 ():
    print("현재 채취 현황 : 미역 {0}개 / 따개비 + 거북손 {1}개".format(mini1_NPC.seaweed_count, mini1_NPC.shell_count))
    print("[{0}] 인어: 미역을 발견했습니다! \n5초 내로 채취하지 않으면 물살에 떠내려 갈지 몰라요.".format(mermaid.name))
    input("\n------------------------------------------------------------------------------\n")
    # second = 현재 시간을 받는거임
    s = time.ctime()
    second = int(s[17])*10+int(s[18])+60
...
def mini1_health():
    ...
    # 돈이 없을 경우
            ...
            input("[미역 사장]: 허... 참... 그래 열심히 일 하다 다친거니 미역 1개! 더이상은 안돼!")
            ...
            if mini1_NPC.seaweed_count >= 1:
                ...
                print("\n[{0}] 인어: 미역 1개와 체력 회복 포션을 교환했으니, {1}개의 미역을 더 채취해야 해요...\n현재 체력이 {2}로 올랐다.".format(mermaid.name, (5-mini1_NPC.seaweed_count), mermaid.HP))
                input("\n------------------------------------------------------------------------------\n")
                break
            ...
            print("[{0}] 인어: 다시 한번 생각해보세요...".format(mermaid.name))
            continue
    return 0
