...
#1이 나왔을 경우 거북손 채취
    if m1_ran ==1 :
        ...
        while True:
            ...
            if m1_choose == '1':
                print("\n------------------------------------------------------------------------------\n")
                m1_turtle = str(input("국산 호미를 사용해 채취하려면 '호미'\n외국산 Homi로 채취하려면 'Homi'를 입력해주세요.\n※작은 따옴표 제외\n"))
...
#2가 나왔을 경우 따개비 채취
        ...
        while True:
            ...
            #따개비  채취 여부 선택
            if m1_choose == "1":
                m1_turtle =input("국산 호미를 사용해 채취하려면 '호미'\n외국산 Homi로 채취하려면 'Homi'를 입력해주세요.\n※작은 따옴표 제외\n")

                #호미 선택
                if m1_turtle == "호미":
                    print("역시 국산 호미! 따개비 채취에 성공했습니다.\n")
                    mermaid.kill += 1
                    mermaid.coin += 2
                    input("------------------------------------------------------------------------------\n")   
                    break
                    ...
                    print("외국산 호미를 사용해 따개비 채취에 성공했습니다.\n")
                    mermaid.kill += 1
                    mermaid.coin += 2
                    input("------------------------------------------------------------------------------\n")   
                    break
    ...
    #게임 시작할 때의 시간이 56 57 58 59 였을 때 후에 받는 after 시간이 1,2,3이 됨으로 60을 한 번 더 더해줘야 함
        if game <= second:
            ...
            if game > timer:
                ...
                mermaid.coin += 10
                seaweed_num = int((50-mermaid.coin)/10)

                #미역을 얼마나 더 모아야 하는지
                if seaweed_num <= 0:
                    print("미역을 다 모았습니다!")
                else:
                    print("미역 채집(1)에 성공했습니다.\n앞으로 채집해야 할 미역 개수: {0}".format(seaweed_num))
                    input("\n------------------------------------------------------------------------------\n")   
...
# 체력 회복 모듈 
def mini1_health():
    print("[%s] 인어: 제 체력이 다 달았어요..."%mermaid.name)
    print("[미역 사장]: 그러게 내가 뭐랬어? 조심하라니깐! \n체력 50 회복 포션을 사두긴 했는데 25코인에 살게!")

    while True:
        mini_h = input("1- 체력 회복 포션을 산다. 2- 사지 않는다.\n")

    #돈이 있을 경우
        if mini_h == 1 and mermaid.coin >= 25:
            mermaid.coin -= 25
            mermaid.health += 50
            print("[미역 사장]: 다시 열심히 일해야겠구만! 고생해~~")
            print("[{0}] 인어: 25코인을 사용했으니, {1}코인 남았네... 다시 일하러 가자.\n현재 체력이 {2}로 올랐다.".format(mermaid.name,mermaid.coin,mermaid.HP))
            break
            
    #돈이 없을 경우
        elif mini_h == 1 and mermaid.coin < 25:
            print("[%s] 인어: 지금까지 모은 코인이 얼마 없어요. 조금만 깎아 주실 수 있나요?"%mermaid.name)
            print("[미역 사장]: 허... 참... 그래 열심히 일 하다 다친거니 10코인!\n더이상은 안돼!")
        
            if mermaid.coin >= 10:
                mermaid.coin -= 10
                mermaid.health += 50
                print("[%s] 인어: 감사합니다~"%mermaid.name)
                print("[미역 사장]: 그래~ 얼른 미역이나 가져와!!! 그래야 코인 주고 돌아가지!")
                print("[{0}] 인어: 25코인을 사용했으니, {1}코인 남았네... 다시 일하러 가자.\n현재 체력이 {2}로 올랐다.".format(mermaid.name,mermaid.coin,mermaid.HP))
                break
            else:
                mermaid.health += 50
                print("[미역 사장]: 어어? 왜 우물쭈물하고 있어! 설마 10코인도 없는거야? \n어휴.. 우리 가게에서  일하다 다친거니 내가 그냥 주지.. 대신 얼른 미역 모아오도록 해!")
                print("[{0}] 인어: 네? 정말요? 감사합니다!\n현재 체력이 {1}로 올랐다".format(mermaoid.name, mermaid.HP))
                break
            break
